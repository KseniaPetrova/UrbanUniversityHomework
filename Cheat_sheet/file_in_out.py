...
print(file.tell)  # положение курсора
file.seek(15)  # смена положения курсора
print(file.readable())  # возвращает True False, если содержимое файла можно читать
print(file.writable())  # возвращает True False, если можно записывать файл
print(file.seekable())  # возвращает True False, если есть возможность перемещать курсор

# вариант чтения файла. в таком варианте файл автоматически закрывается
name = 'poem.txt'
with open(name, encoding='utf-8') as file:  # указывается операция, затем в каком виде она должна быть
    for line in file:  # считывает текст построчно
        for char in line:  # считывает текст посимвольно
            print(char, end='')  # end в данном случаем не даст переносить на каждую новую строку каждый символ
