...
class Cats:
    ...
    # object.__delattr__(self, name) - удаление атрибута из объекта. Этот метод вызывается, когда атрибут удаляется с помощью оператора del
    # object.__dict__() - Возвращает словарь, содержащий атрибуты объекта в виде пар "ключ-значение"
    # object.__dir__(self) - Возвращает список всех атрибутов объекта, включая методы и переменные.
    # object.__doc__() - Возвращает строку документации объекта, если она предоставлена.
    # object.__format__(self, format_specifier) - Определяет форматирование объекта. Этот метод позволяет контролировать, как объект будет отображаться при форматировании строк.
    # object.__getattribute__(self, name) - Возвращает значение атрибута объекта
    # object.__hash__(self) - Возвращает хеш-код объекта. Этот метод используется для хеширования объектов
    # object.__module__() - Возвращает имя модуля, в котором определен класс или функция
    # object.__reduce__(self) - Возвращает кортеж, содержащий аргументы, необходимые для восстановления объекта
    # object.__setattr__(self, name, value) - Устанавливает значение атрибута объекта
    # object.__sizeof__(self) - Возвращает размер объекта в байтах.
...
print(Cats.__mro__)  # (<class '__main__.Cats'>, <class 'object'>)
