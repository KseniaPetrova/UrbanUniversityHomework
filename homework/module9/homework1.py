...
При переборе функций записывайте в словарь reuslts результат работы этой функции
...
При попытке передачи, например, списка из строк, некоторые функции могут работать
некорректно или вовсе не работать. Используйте списки чисел."""

def apply_all_func(int_list: list[int or float], *functions):
    reuslts: dict = {}
    for func in functions:
        reuslts[func.__name__] = func(int_list)  #передаем функции аргумент, чтобы получить результат работы
    return reuslts
